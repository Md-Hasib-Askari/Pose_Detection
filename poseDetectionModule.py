...
def findCoordinates(img, p1, lmList, draw=True):
    x1, y1 = lmList[p1][1:]
    if draw:
...
class PoseDetector:
    def __init__(self, static_image_mode=False, model_complexity=1, smooth_landmarks=True, enable_segmentation=False, smooth_segmentation=True, min_detection_confidence=0.5, min_tracking_confidence=0.5):
        ...
        self.pose = self.mpPose.Pose(self.mode, self.modelComplexity, self.smooth, enable_segmentation, smooth_segmentation, self.detectionCon, self.trackCon)
